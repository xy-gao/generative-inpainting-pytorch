...
class ContextualAttention(nn.Module):
    ...
    def forward(self, f, b, mask=None):
        ...
        if mask is None:
            ...
            mask = F.interpolate(
                mask, scale_factor=1.0 / (4 * self.rate), mode="nearest"
            )
        ...
        h_add = (
            torch.arange(int_fs[2])
            .view([1, 1, int_fs[2], 1])
            .expand(int_fs[0], -1, -1, int_fs[3])
        )
        w_add = (
            torch.arange(int_fs[3])
            .view([1, 1, 1, int_fs[3]])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--imageB", default="", type=str, help="Image B is reconstructed with image A."
    )
    parser.add_argument(
        "--imageOut",
        default="result.png",
